...
class Controller_Form extends Controller
{
    ...
    public function action_addBook(){
        ...
        $db = DB::getInstance();
        $stmt = $db->prepare('INSERT INTO `library`.`books` (`name`, `img`, `year`, `description`) VALUES (:name, :img, :year, :description)');
        $stmt->execute([
        ...
        foreach($authors as $author){
            ...
            $stmt = $db->prepare("INSERT INTO `library`.`authors` SET `name` =  :name ON DUPLICATE KEY UPDATE `id` = LAST_INSERT_ID(`id`), name = :name");
            $stmt->execute(['name' => $author]);
            $authorId = $db->lastInsertId();
            $db->query('INSERT INTO `library`.`booksAuthors` (`author_id`, `book_id`) VALUES ("'. $authorId .'", "'. $bookId .'")');
        }
        $this->redirect();
    }
